...
export default function MentorDashboardPlus() {
  ...
  return createPortal(
    <section className="mb-6 rounded-2xl border-4 border-violet-950 bg-violet-950 p-5 text-white shadow-[6px_6px_0_#2e1065]" data-testid="mentor-dashboard-plus">
      ...
      {tab === "codes" && <div className="mt-5 rounded-xl border-2 border-violet-700 bg-white p-4 text-violet-950"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">KODE TERAKHIR DIBUAT</h3><p className="mt-1 text-xs text-slate-600">Kode dibuat dari panel <b>KODE SISWA</b> di atas. Di sini hanya untuk melihat dan menyalin hasil.</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{codes.map((item) => <div key={item.id} className="rounded-lg border-2 border-violet-200 bg-yellow-50 p-3"><code className="font-black text-violet-950">{item.code}</code><div className="mt-1 text-[10px] font-bold">{levelLabel[item.level]} · SEKALI PAKAI</div></div>)}{!codes.length && <p className="col-span-full py-8 text-center text-xs text-slate-500">Belum ada kode di sesi ini. Generate dari Ringkasan.</p>}</div></div>}
