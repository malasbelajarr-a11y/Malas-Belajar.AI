...
function readLocalResources(): Resource[] {
  try {
...
function saveLocalResources(items: Resource[]) {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(items.slice(0, 100)));
  } catch {
    // localStorage is only a cache; API data remains authoritative.
  }
}

function mergeResources(apiItems: Resource[]) {
  const map = new Map<string, Resource>();
  [...readLocalResources(), ...apiItems].forEach((item) => map.set(item.id, item));
  const result = Array.from(map.values());
  saveLocalResources(result);
  return result;
}
...
export default function LockerSubtestFix() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([module, added, apiResources]) => {
        const moduleQuestions = Array.isArray(module?.questions) ? module.questions : [];
        setQuestions([...moduleQuestions, ...(Array.isArray(added) ? added : [])]);
        setResources(mergeResources(Array.isArray(apiResources) ? apiResources : []));
      })
  ...
  const uploadWacawaci = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        created = await apiUpload<Resource>(
        ...
        created = await apiPost<Resource>("/wacawaci/resources", {
          mentor_code: "CECEKOKOMLS",
          kind,
          title,
          description,
      ...
      setResources(mergeResources(Array.isArray(fresh) ? fresh : [created]));
      setTitle("");
      setDescription("");
      setUrl("");
      setFile(null);
      toast.success("Materi Wacawaci berhasil ditambahkan.");
    } catch (error: any) {
      toast.error(error?.body?.message || error?.message || "Upload gagal.");
    }
  };
        ...
        <header className="sticky top-0 z-20 flex items-center justify-between border-b-4 border-violet-950 bg-yellow-300 p-4">
          <div>
            ...
            <h1 className="text-2xl font-black text-violet-950">
              {mentor ? "KONTEN MENTOR — 7 SUBTES" : active === "rodi" ? "RODI — PILIH LOKER SUBTES" : "WACAWACI — PILIH LOKER SUBTES"}
            </h1>
          </div>
          <button className="rounded-lg border-2 border-violet-950 bg-white p-2" onClick={closeOverlay} aria-label="Tutup">
            <X />
          </button>
        </header>

        {mentor && (
          <div className="border-b-4 border-violet-950 bg-white p-4">
            <div className="flex flex-wrap gap-2">
              <button onClick={() => setMentorTab("rodi")} className={`rounded-lg border-2 border-violet-950 px-5 py-3 font-black ${mentorTab === "rodi" ? "bg-yellow-300" : "bg-white"}`}>
                <Plus className="mr-1 inline h-4 w-4" /> TAMBAH SOAL RODI
              </button>
              <button onClick={() => setMentorTab("wacawaci")} className={`rounded-lg border-2 border-violet-950 px-5 py-3 font-black ${mentorTab === "wacawaci" ? "bg-pink-300" : "bg-white"}`}>
                <Upload className="mr-1 inline h-4 w-4" /> TAMBAH WACAWACI
              </button>
            </div>
          </div>
        )}
        ...
        {mentor && mentorTab === "rodi" && (
          <section className="border-b-4 border-violet-950 bg-violet-50 p-5">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="rounded-lg border-2 border-violet-950 bg-white p-3 font-bold">
                <option>Mudah</option><option>Sedang</option><option>Sulit</option><option>Sangat Sulit</option>
              </select>
              ...
              <div className="md:col-span-2">
                <p className="mb-2 font-black">Pilihan jawaban A–E</p>
                <div className="grid gap-2 md:grid-cols-2">
                  {options.map((value, index) => (
                    <input key={index} value={value} onChange={(e) => setOptions((prev) => prev.map((x, i) => i === index ? e.target.value : x))} placeholder={`${String.fromCharCode(65 + index)}. pilihan`} className="rounded-lg border-2 border-violet-950 bg-white p-3" />
                  ))}
                </div>
              </div>
              <select value={correctOption} onChange={(e) => setCorrectOption(e.target.value)} className="rounded-lg border-2 border-violet-950 bg-white p-3 font-bold">
                <option value="">Jawaban benar (opsional)</option>
                {options.map((value, index) => <option key={index} value={index}>{String.fromCharCode(65 + index)}{value ? `. ${value}` : ""}</option>)}
              </select>
          ...
          <section className="border-b-4 border-violet-950 bg-violet-50 p-5">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <select value={kind} onChange={(e) => setKind(e.target.value as Kind)} className="rounded-lg border-2 border-violet-950 bg-white p-3 font-bold">
                <option value="video">Video</option><option value="module">Modul</option>
              </select>
              <select value={level} onChange={(e) => setLevel(e.target.value as Level)} className="rounded-lg border-2 border-violet-950 bg-white p-3 font-bold">
              ...
              <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="URL video/PDF (opsional)" className="rounded-lg border-2 border-violet-950 bg-white p-3 md:col-span-2" />
              ...
              <button onClick={uploadWacawaci} className="rounded-lg border-2 border-violet-950 bg-pink-300 px-5 py-3 font-black md:col-span-2"><Upload className="mr-2 inline h-4 w-4" /> UPLOAD WACAWACI</button>
            </div>
          </section>
        )}
        ...
        <section className="p-5">
          ...
          {active === "wacawaci" && (
            <div className="border-t-4 border-violet-950 bg-violet-50 p-5">
              ...
              {loading ? <p className="py-10 text-center font-bold">Memuat…</p> : (
                <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {visibleResources.length ? visibleResources.map((resource) => (
                    <article key={resource.id} className="rounded-xl border-2 border-violet-200 bg-white p-4">
                      <span className="rounded bg-pink-100 px-2 py-1 text-xs font-black">{resource.kind}</span>
                      <h3 className="mt-3 font-black text-violet-950">{resource.title}</h3>
                      <p className="mt-2 text-sm text-slate-600">{resource.description}</p>
                      {resource.url ? <a href={resource.url} target="_blank" rel="noreferrer" className="mt-4 inline-block font-black text-pink-600 underline">BUKA MATERI →</a> : null}
                    </article>
