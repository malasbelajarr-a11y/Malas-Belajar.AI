...
const writeStored = (items: Resource[]) => {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(items.slice(0, 100))); } catch { /* API copy remains available */ }
};
  ...
  useEffect(() => {
    ...
    const upload = async (event: Event) => {
      ...
      try {
        ...
        const fileInput = input("admin-resource-file-input");
    ...
    return () => {
      observer.disconnect();
      document.removeEventListener("click", upload, true);
    };
  }, []);
  return null;
}
