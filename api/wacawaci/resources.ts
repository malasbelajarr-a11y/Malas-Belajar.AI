...
async function driveResources(): Promise<Resource[]> {
  ...
  while (queue.length) {
    ...
    for (const item of children) {
      ...
      result.push({ id: `drive-${item.id}`, kind, title: item.name, description: path, url: item.webViewLink || `https://drive.google.com/open?id=${item.id}`, is_public: true, created_by: "Google Drive Wacawaci", level: "", subtest });
    }
  }
  return result;
}
export default async function handler(req: any, res: any) {
  try {
    if (req.method === "GET") return res.status(200).json([...(await driveResources()), ...memoryResources]);
    ...
    const resource: Resource = { id: `res-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, kind, title, description, url, is_public: true, created_by: "Mentor Malas Belajar", level, subtest };
