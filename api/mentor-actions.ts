...
function requireDatabase(){if(!configured())throw new Error('Penyimpanan belum aktif: SUPABASE_URL dan SUPABASE_SERVICE_ROLE_KEY belum tersedia di Vercel. Data mentor tidak akan disimpan sementara konfigurasi ini belum ada.');}
async function db(path:string,init:RequestInit={}){
  requireDatabase();
  const base=String(process.env.SUPABASE_URL||'').replace(/\/$/,''); const key=String(process.env.SUPABASE_SERVICE_ROLE_KEY||'');
  const r=await fetch(`${base}/rest/v1/${path}`,{...init,signal:AbortSignal.timeout(8000),headers:{apikey:key,Authorization:`Bearer ${key}`,'Content-Type':'application/json',...(init.headers||{})}});
...
async function saveResource(data:any){await db('wacawaci_resources',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(data)});return true}
...
export default async function handler(req:VercelRequest,res:VercelResponse){
  try{
    ...
    if(action==='questions'){
      ...
      await saveResource({id:q.id,kind:'mentor_question',title:`${q.chapter_label} #${q.number}`,description:JSON.stringify(q),url:q.file_url||q.video_url||'',is_public:true,created_by:'mentor',level:q.level});
    ...
    if(action==='bank'){
      ...
      const questions=parseRows(await db('wacawaci_resources?kind=eq.mentor_question&select=*&order=created_at.desc'));
      const tryouts=parseRows(await db('wacawaci_resources?kind=eq.mentor_tryout&select=*&order=created_at.desc'));
    ...
    if(action==='tryouts'){
      ...
      await saveResource({id:t.id,kind:'mentor_tryout',title:t.title,description:JSON.stringify(t),url:'',is_public:true,created_by:'mentor',level:t.level}); return res.status(201).json(t);
    }
    if(action==='live'){
      if(req.method==='GET')return res.status(200).json(parseRows(await db('wacawaci_resources?kind=eq.live_class&select=*&order=created_at.desc')));
      if(req.method!=='POST')return res.status(405).json({detail:'Method tidak didukung.'}); if(!validMentor(body.mentor_code))return res.status(403).json({detail:'Kode mentor tidak valid.'});
      ...
      await saveResource({id:item.id,kind:'live_class',title:item.title,description:JSON.stringify(item),url:item.youtube_url,is_public:true,created_by:'mentor',level:item.level}); return res.status(201).json(item);
    }
