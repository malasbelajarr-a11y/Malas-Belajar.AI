...
async function auth(req: any, res: any, path: string) {
  ...
  if (path === "/api/auth/register" && req.method === "POST") {
    ...
    const level = levelFromCode(requestedCode); if (!level) return res.status(400).json({ detail: "Kode akses tidak valid." });
    if (await accessCodeAlreadyUsed(requestedCode) || generatedCodes.get(requestedCode)?.used) return res.status(409).json({ detail: "Kode akses ini sudah pernah dipakai." });
  ...
  if (path === "/api/auth/demo" && req.method === "POST") {
    ...
    const students = await listStudents(); const demo = students.find((s) => s.id === (requested === "nguli" ? "usr-demo-1" : requested === "mandor" ? "usr-mandor-1" : "usr-spv-1"));
    if (!demo) return res.status(404).json({ detail: "Akun demo tidak tersedia." });
    setCookie(res, demo); return res.status(200).json(publicStudent(demo));
  }
  if (path === "/api/auth/level" && (req.method === "PATCH" || req.method === "POST")) {
    const session = readSession(req); const email = String(body.email || session?.email || "").trim().toLowerCase(); const target = String(body.level || "").trim().toLowerCase() as Level; const code = String(body.access_code || body.code || "").trim().toUpperCase();
    ...
    const updated = await markAccessCodeUsed(student, code, target); const entry = generatedCodes.get(code); if (entry) { entry.used = true; entry.used_by = student.id; }
    setCookie(res, updated); return res.status(200).json(publicStudent(updated));
  }
  return null;
}
async function admin(req: any, res: any, path: string) {
  ...
  if (path === "/api/admin/access-codes" && req.method === "POST") {
    ...
    const level = String(body.level || "nguli").toLowerCase() as Level; const count = Math.max(1, Math.min(50, Number(body.count) || 3));
    if (!["nguli", "mandor", "supervisor"].includes(level)) return res.status(400).json({ detail: "Level tidak valid." });
    const prefix = level === "nguli" ? "NGU" : level === "mandor" ? "MAN" : "SPV"; const out = [];
    for (let i = 0; i < count; i++) { let code = ""; do { code = `MLS-${prefix}-${crypto.randomInt(1000, 10000)}`; } while (generatedCodes.has(code)); generatedCodes.set(code, { level, used: false, used_by: "" }); out.push({ id: `code-${Date.now()}-${i}`, code, level, used: false, used_by: "" }); }
