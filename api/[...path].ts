...
const seedCodes: Array<[string, Level]> = [
  ["NGULI-MLS", "nguli"],
  ["MLS2026", "nguli"],
  ["MLS-NGU-9921", "nguli"],
  ["MANDOR-MLS", "mandor"],
...
function bodyOf(req: VercelRequest): any {
  if (!req.body) return {};
...
function readSession(req: VercelRequest): any | null {
...
async function handleAuth(req: VercelRequest, res: VercelResponse, path: string) {
  ...
  if (path === "/api/auth/me" && req.method === "GET") {
    ...
    const student = students.find((s) => s.id === String(session.id));
  ...
  if (path === "/api/auth/level" && req.method === "PATCH") {
    const session = readSession(req);
...
async function handleAdmin(req: VercelRequest, res: VercelResponse, path: string) {
  if (path === "/api/admin/students" && req.method === "GET") {
    const students = await listStudents();
    return res.status(200).json(students.map(publicStudent));
  }
  if (path === "/api/admin/access-codes" && req.method === "POST") {
    ...
    for (let i = 0; i < count; i++) {
      let code = "";
      do { code = `MLS-${prefix}-${crypto.randomInt(1000, 10000)}`; } while (generatedCodes.has(code));
  ...
  if (path === "/api/admin/students" && req.method === "PATCH") {
    ...
    const { setStudentActive } = await import("./_lib/studentStore");
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (path.startsWith("/api/auth/")) {
      const result = await handleAuth(req, res, path);
      if (result !== null) return result;
    }
    if (path.startsWith("/api/admin/")) {
      const result = await handleAdmin(req, res, path);
      if (result !== null) return result;
    }
