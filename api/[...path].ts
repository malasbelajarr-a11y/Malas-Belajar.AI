...
async function auth(req: any, res: any, path: string) {
  ...
  if (path === "/api/auth/demo" && req.method === "POST") {
    ...
    if (!["nguli", "mandor", "supervisor"].includes(requested)) return res.status(400).json({ detail: "Level demo tidak valid." });
...
export default async function handler(req: any, res: any) {
  ...
  req.url = path + (String(req.url || "").includes("?") ? `?${String(req.url).split("?")[1]}` : "");
  try {
    if (path.startsWith("/api/auth/")) {
      const result = await auth(req, res, path);
      if (result !== null) return result;
    }
    if (path.startsWith("/api/admin/")) {
      const result = await admin(req, res, path);
      if (result !== null) return result;
    }
    return app(req, res);
  } catch (error) {
